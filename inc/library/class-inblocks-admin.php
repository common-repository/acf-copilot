...
if ( ! class_exists( 'INBLOCKS_Admin' ) ) {

	class INBLOCKS_Admin {
		/**
		 * Consturtor.
		 */
		...
		public function check_nonce_token() {
			if ( ! check_ajax_referer( 'inblocks-ajax-nonce', 'security', false ) ) {
				return false;
			}
		...
		public function get_invalid_nonce_token() {
			...
			$message    = esc_html__( '%1$s! Contact us @ %2$s.', 'instagram-blocks' );
			$values_arr = array(
				'<strong>' . esc_html__( 'Invalid security token', 'instagram-blocks' ) . '</strong>',
				'<a href="mailto:contact@' . INBLOCKS_PLUGIN_DOMAIN . '">contact@' . INBLOCKS_PLUGIN_DOMAIN . '</a>',
			);
		...
		public function get_invalid_user_cap() {
			...
			$message    = esc_html__( '%1$s! Current user does not have the capabilities to access this function.', 'instagram-blocks' );
			$values_arr = array( '<strong>' . esc_html__( 'Access denied', 'instagram-blocks' ) . '</strong>' );
